...
<Tip>
    After following this guide, provide Connective Health with all details from the [finish S3](#finish-s3-setup) or [finish Azure](#finish-azure-setup) sections
</Tip>

### File Delivery Location Template

Customers are able to decide the location within their cloud storage (AWS S3 or Azure Blob Storage) where the files are delivered to using a configurable template which
...
2. The name of your storage account and container—we will need both staging/test and production environments
3. The app registration Application (client) ID
4. The app registration Directory (tenant) ID
5. The storage account's blob service endpoint URL
6. The storage account's container name
